...
while True:
    ...
    retR, cornersR = cv2.findChessboardCorners(
        image=grayR,  
        patternSize=(CHECKERBOARD['cols'], CHECKERBOARD['rows']),
        corners=None)
    retL, cornersL = cv2.findChessboardCorners(
        grayL, 
        (CHECKERBOARD['cols'], CHECKERBOARD['rows']),
        None)

    # If corners are detected in left and right image then we save it.
    if (retR == True) and (retL == True) and timer <= 0:
        count += 1
        cv2.imwrite(output_path+'stereoR/img-{:04d}.png'.format(count), frameR)
        cv2.imwrite(output_path+'stereoL/img-{:04d}.png'.format(count), frameL)
